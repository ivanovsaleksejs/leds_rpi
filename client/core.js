var Ajax = {
    ...
    Request : function(url, params) {
        ...
        if (typeof params.nocache !== "undefined" && params.nocache) {
            var d = new Date
            params.data.nocache = d.getTime() + Math.round(d.getMilliseconds() / 100)
        }
        params.data = Ajax.parse_data(params.data)

        xmlhttp.open(params.method, url + (params.method == "GET" ? ("?" + params.data) : ""), params.async)
        xmlhttp.setRequestHeader("X-Requested-With","XMLHttpRequest")
        xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded")
        params.method == "GET" ? xmlhttp.send() : xmlhttp.send(params.data)
        return xmlhttp
    },
...
var Model = {
    ...
    request  : function(method, model, data) {
        if (method == "GET" && Router.is_static) {
